...
resource "aws_instance" "kubernetes_workers" {
  ...
  subnet_id              = aws_subnet.kubernetes_subnet.id                   # Connecter l'instance au sous réseau
  vpc_security_group_ids = [aws_security_group.kubernetes_security_group.id] # Attachez le groupe de sécurité à cette instance
